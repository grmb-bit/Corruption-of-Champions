...
public class PlayerEvents extends BaseContent implements TimeAwareInterface {
		...
		public function timeChange():Boolean {
			...
			//Normal
			if (player.hasPerk(PerkLib.WellAdjusted) == false) {
			...
			else if (player.biggestLactation() > 0 && !player.hasStatusEffect(StatusEffects.Feeder) && player.hasPerk(PerkLib.MilkMaid) == false && player.pregnancyIncubation == 0) {
			...
			if (player.hasStatusEffect(StatusEffects.CuntStretched)) { //Cunt stretching stuff
				...
				if (player.vaginas.length > 0) {
					if (player.hasPerk(PerkLib.FerasBoonWideOpen) == false) {
			...
			if (player.hasKeyItem("Ruby Heart") >= 0) { //Regain slime core
				if (player.hasStatusEffect(StatusEffects.SlimeCraving) && player.hasPerk(PerkLib.SlimeCore) == false && player.isGoo() && player.gooScore() >= 4 && player.vaginalCapacity() >= 9000 && player.skinAdj == "slimy" && player.skinDesc == "skin" && player.lowerBody == LowerBody.GOO) {
			...
			if (player.hasKeyItem("Ruby Orb") >= 0) { //Regain DARK slime core
				if (player.hasStatusEffect(StatusEffects.SlimeCraving) && player.hasPerk(PerkLib.DarkSlimeCore) == false && player.isGoo() && player.gooScore() >= 4 && player.vaginalCapacity() >= 9000 && player.skinAdj == "slimy" && player.skinDesc == "skin" && player.lowerBody == LowerBody.GOO) {
					outputText("\nAs you adjust to your new, goo-like body, you remember the ruby heart you expelled so long ago.  As you reach to pick it up, it quivers and pulses with a warm, cheerful light.  Your fingers close on it and the nucleus slides through your palm, into your body!\n\n");
			...
			if (player.hasPerk(PerkLib.FluidBody) == false && player.isGoo() && (player.gooScore() >= 11 || player.magmagooScore() >= 13 || player.darkgooScore() >= 13)) {
			...
			if (CoC.instance.model.time.hours > 23) { //Once per day
				...
				if (player.hasStatusEffect(StatusEffects.DragonBreathCooldown) && player.hasPerk(PerkLib.DraconicLungsFinalForm) == false) player.removeStatusEffect(StatusEffects.DragonBreathCooldown);
				if (player.hasStatusEffect(StatusEffects.DragonDarknessBreathCooldown) && player.hasPerk(PerkLib.DraconicLungs) == false) player.removeStatusEffect(StatusEffects.DragonDarknessBreathCooldown);
				if (player.hasStatusEffect(StatusEffects.DragonFireBreathCooldown) && player.hasPerk(PerkLib.DraconicLungs) == false) player.removeStatusEffect(StatusEffects.DragonFireBreathCooldown);
				if (player.hasStatusEffect(StatusEffects.DragonIceBreathCooldown) && player.hasPerk(PerkLib.DraconicLungs) == false) player.removeStatusEffect(StatusEffects.DragonIceBreathCooldown);
				if (player.hasStatusEffect(StatusEffects.DragonLightningBreathCooldown) && player.hasPerk(PerkLib.DraconicLungs) == false) player.removeStatusEffect(StatusEffects.DragonLightningBreathCooldown);
				...
				if (player.hasPerk(PerkLib.JobSoulCultivator) == false && (player.soulforce < player.maxSoulforce())) {
					player.soulforce += 50;
				...
				if (player.hasPerk(PerkLib.JobSorcerer) == false && player.hasPerk(PerkLib.JobElementalConjurer) == false && (player.mana < player.maxMana())) {
					player.mana += 150;
		...
		private function hourlyCheckRacialPerks():Boolean {
			...
			if (player.demonScore() >= 10 || player.hasPerk(PerkLib.Phylactery)) { //Check for being a demon enough
				if (player.hasPerk(PerkLib.DemonEnergyThirst) == false) {
			...
			if ((player.tailType == Tail.CAT || player.tailType == Tail.MANTICORE_PUSSYTAIL || player.tailType == Tail.BURNING) && (player.lowerBody == LowerBody.CAT || player.lowerBody == LowerBody.LION) && (player.arms.type == Arms.CAT || player.arms.type == Arms.LION || player.arms.type == Arms.DISPLACER)) { //Check for gain of cat agility - requires legs, tail, and arms
				if (player.hasPerk(PerkLib.Flexibility) == false) {
			...
			else if (player.hasPerk(PerkLib.Flexibility) && player.perkv4(PerkLib.Flexibility) == 0 && player.hasPerk(PerkLib.CatlikeNimbleness) == false) { //Remove flexibility perk if not meeting requirements
			...
			//Ghost-slinger perk
			if (player.poltergeistScore() >= 12 && player.hasPerk(PerkLib.Ghostslinger) == false) {
			...
			else if (player.poltergeistScore() < 12 && player.hasPerk(PerkLib.Ghostslinger)) {// && player.hasPerk(PerkLib.LizanMarrow) == false
			...
			//Knowledge is power
			if (player.ratatoskrScore() >= 12 && player.hasPerk(PerkLib.KnowledgeIsPower) == false) {
			...
			else if (player.ratatoskrScore() < 12 && player.hasPerk(PerkLib.KnowledgeIsPower)) {// && player.hasPerk(PerkLib.LizanMarrow) == false
			...
			if (flags[kFLAGS.VENOM_TIMES_USED] >= 50 && player.hasPerk(PerkLib.ImprovedVenomGland) == false) {
			...
			if (player.kitsuneScore() >= 10) { //Check for being a kitsune enough
				if (player.hasPerk(PerkLib.KitsuneEnergyThirst) == false) {
			...
			if (player.kitsuneScore() >= 9 ){
				if(player.cor >= 50 && player.hasPerk(PerkLib.DarkenedKitsune) == false) { //Check for being kitsune enough + high enough corruption
			...
			*/
			if (player.hasPerk(PerkLib.DarkenedKitsune) >= 0){ //Just in case someone else has it, this will remove it. No longer in use. Read above.
				player.removePerk(PerkLib.DarkenedKitsune);
			}
			...
			if ((player.tailType == Tail.LIZARD && player.lowerBody == LowerBody.LIZARD && player.arms.type == Arms.LIZARD) || (player.hasPerk(PerkLib.LizanRegeneration) == false && player.hasPerk(PerkLib.LizanMarrow))) { //Check for gain of lizan regeneration - requires legs, arms and tail
				if (player.hasPerk(PerkLib.LizanRegeneration) == false) {
			...
			else if (player.tailType != Tail.LIZARD && player.lowerBody != LowerBody.LIZARD && player.arms.type != Arms.LIZARD && player.hasPerk(PerkLib.LizanRegeneration) && player.perkv4(PerkLib.LizanRegeneration) == 0 && player.hasPerk(PerkLib.LizanMarrow) == false) { //Remove lizan regeneration perk if not meeting requirements && player.hasPerk(PerkLib.HydraRegeneration) == false
			...
			if ((player.salamanderScore() >= 9 || player.phoenixScore() >= 10) && (!player.hasPerk(PerkLib.Lustzerker))) { //Check for gain of lustzerker - requires legs, arms and tail
				if (player.hasPerk(PerkLib.Lustzerker) == false) {
			...
			else if (player.salamanderScore() < 9 && player.phoenixScore() < 10 && player.hasPerk(PerkLib.Lustzerker) && player.perkv4(PerkLib.Lustzerker) == 0 && player.hasPerk(PerkLib.SalamanderAdrenalGlands) == false) { //Remove lustzerker perk if not meeting requirements
			...
			if (player.poltergeistScore() >= 12 && player.hasPerk(PerkLib.PhantomShooting) == false) {
				outputText("\n");
			...
			else if (player.poltergeistScore() < 12 && player.hasPerk(PerkLib.PhantomShooting)) {// && player.hasPerk(PerkLib.LizanMarrow) == false
			...
			if (player.satyrScore() >= 4 && player.balls > 0) {
				if (player.hasPerk(PerkLib.SatyrSexuality) == false) {
			...
			if (player.demonScore() >= 6) {
				if (player.hasPerk(PerkLib.DarkCharm) == false) {
			...
			else if (player.hasPerk(PerkLib.DarkCharm) && player.perkv4(PerkLib.DarkCharm) == 0 && player.hasPerk(PerkLib.BlackHeart) == false) {
			...
			if (player.redpandaScore() >= 6) {
				if (player.hasPerk(PerkLib.JunglesWanderer) == false) {
			...
			//Freezing Breath
			if (player.faceType == Face.WOLF && player.hasPerk(PerkLib.FreezingBreath) == false && player.hasKeyItem("Gleipnir Collar") >= 0) {
			...
			//Fenrir Soulbite
			if (player.faceType == Face.WOLF && player.hasPerk(PerkLib.FenrirSpiritstrike) == false && player.hasKeyItem("Gleipnir Collar") >= 0) {
			...
			//Cold Affinity
			if (player.hasPerk(PerkLib.WhaleFat) && player.hasPerk(PerkLib.ColdAffinity) == false) {
			...
			if (player.yetiScore() >= 6 && player.hasPerk(PerkLib.ColdAffinity) == false) {
			...
			if ((player.melkieScore() >= 8 || player.frostWyrmScore() >= 10) && player.hasPerk(PerkLib.ColdAffinity) == false) {
				outputText("\nYou suddenly no longer feel the cold so you guess you finally got acclimated to the icy winds of the glacial rift. You feel at one with the cold. So well that you actually developed icy power of your own.\n\n(<b>Gained Perks: Cold Affinity</b>)\n");
				player.createPerk(PerkLib.ColdAffinity, 0, 0, 0, 0);
				needNext = true;
			}
			...
			//Icy flesh
			if (player.hasPerk(PerkLib.IcyFlesh) && player.yukiOnnaScore() < 14) {
			...
			//Aquatic Affinity
			if (player.lowerBody == LowerBody.ORCA && player.arms.type == Arms.ORCA && player.tailType == Tail.ORCA && player.ears.type == Ears.ORCA && player.hasPerk(PerkLib.AquaticAffinity) == false) {
			...
			//Fire Affinity
			if ((player.salamanderScore() >= 4 || player.phoenixScore() >= 10 || player.hellcatScore() >= 10 || player.firesnailScore() >= 15 || (player.mouseScore() >= 12 && player.lowerBody == LowerBody.HINEZUMI && player.arms.type == Arms.HINEZUMI && player.tailType == Tail.HINEZUMI)) && player.hasPerk(PerkLib.FireAffinity) == false) {
			...
			//Lightning Affinity
			if ((player.raijuScore() >= 7 || player.thunderbirdScore() >= 12) && player.hasPerk(PerkLib.LightningAffinity) == false) {
			...
			else if (player.raijuScore() < 7 && player.hasPerk(PerkLib.LightningAffinity) && player.hasStatusEffect(StatusEffects.IsRaiju) && !player.hasStatusEffect(StatusEffects.IsThunderbird)) {
			...
			if (player.thundermantis() >= 10 && player.tailType == Tail.THUNDERBIRD && player.hasPerk(PerkLib.LightningAffinity) == false) {
			...
			if (player.lowerBody == LowerBody.YETI && player.arms.type == Arms.YETI && player.hasPerk(PerkLib.BigHandAndFeet) == false) {
			...
			//Wendigo stuff
			if (player.hasStatusEffect(StatusEffects.WendigoPsychosis) && player.hasPerk(PerkLib.EndlessHunger) == false && ((flags[kFLAGS.HUNGER_ENABLED] > 0 && player.hunger <= 0) || (flags[kFLAGS.HUNGER_ENABLED] <= 0 && player.lust >= player.maxLust()))) SceneLib.glacialRift.wendigoScene.becomeWendigo();
			//Gazer perks
			...
			//Necromancy perk
			if (((player.tailType == Tail.CAT && player.tailCount == 2) || player.tailType == Tail.NEKOMATA_FORKED_2_3 || player.tailType == Tail.NEKOMATA_FORKED_1_3) && player.hasPerk(PerkLib.Necromancy) == false) {
			...
			//Elven Sense
			if ((player.eyes.type != Eyes.ELF || player.ears.type != Ears.ELVEN) && player.hasPerk(PerkLib.ElvenSense) && player.hasPerk(PerkLib.ElvishPeripheralNervSys) == false) {
			...
			//Ferocity
			if (player.orcScore() < 11 && player.hasPerk(PerkLib.Ferocity) && player.hasPerk(PerkLib.OrcAdrenalGlandsFinalForm) == false) {
			...
			//Titanic Strength
			if ((player.hydraScore() >= 14 || player.oniScore() >= 12 || player.orcaScore() >= 17 || player.scyllaScore() >= 12 || player.frostWyrmScore() >= 10 || player.cyclopScore() >= 12) && player.tallness >= 80 && player.hasPerk(PerkLib.TitanicStrength) == false) {
			...
			//Enigma
			if (player.sphinxScore() >= 14 && player.hasPerk(PerkLib.Enigma) == false) {
			...
			if (player.cowScore() >= 10 && player.hasPerk(PerkLib.LactaBovineImmunity) == false) {
			...
			//Bull Strength
			if ((player.minotaurScore() >= 15 || player.cowScore() >= 15) && player.hasPerk(PerkLib.BullStrength) == false) {
			...
			//Soul Sense
			if (player.maxSoulforce() >= 200 && player.hasPerk(PerkLib.SoulApprentice) && player.hasPerk(PerkLib.SoulSense) == false) {
			...
			if ((player.isGoblinoid()) && player.hasPerk(PerkLib.GoblinoidBlood) == false) {
			...
			if ((player.isGoblinoid()) && player.hasPerk(PerkLib.BouncyBody) == false) {
			...
			else if ((!player.isGoblinoid()) && player.hasPerk(PerkLib.BouncyBody) && player.hasPerk(PerkLib.NaturalPunchingBagFinalForm) == false) {
			...
			//		if (player.level >= 24 && player.hasPerk(PerkLib.SoulApprentice) && !player.hasStatusEffect(StatusEffects.TribulationCountdown) && player.hasPerk(PerkLib.HclassHeavenTribulationSurvivor) == false) {
			//			outputText("\nPLACEHOLDER TEXT 1\n");
			...
			//			if (player.statusEffectv1(StatusEffects.TribulationCountdown) <= 1 && player.hasPerk(PerkLib.HclassHeavenTribulationSurvivor) == false) {
			...
			//	needNext = true;
			//			}
			//	else if (player.statusEffectv1(StatusEffects.TribulationCountdown) <= 1 && player.hasPerk(PerkLib.HclassHeavenTribulationSurvivor) == false) {
			//		player.removeStatusEffect(StatusEffects.TribulationCountdown);
			//		outputText("\nYou feel a tingling in your nethers... at last full sensation has returned to your groin.  <b>You can masturbate again!</b>\n");
			//		needNext = true;
			//	}
			...
			//Tail Hunger
			if (player.tailType == Tail.MANTICORE_PUSSYTAIL && player.hasPerk(PerkLib.ManticoreCumAddict) == false) {
			...
			//Milk Hunger
			if (player.rearBody.type == RearBody.DISPLACER_TENTACLES && player.hasPerk(PerkLib.DisplacerMilkAddict) == false) {
			...
			if (flags[kFLAGS.BASILISK_RESISTANCE_TRACKER] >= 100 && player.hasPerk(PerkLib.BasiliskResistance) == false) {
			...
			if (flags[kFLAGS.TIMES_TRANSFORMED] >= 25 && player.hasPerk(PerkLib.GeneticMemory) == false) {
			...
			if (flags[kFLAGS.TIMES_TRANSFORMED] >= 100 && player.hasPerk(PerkLib.TransformationResistance) == false) {
			...
			if ((player.horns.type == Horns.BICORN || player.horns.type == Horns.UNICORN) && player.cor > 89 && player.hasPerk(PerkLib.AvatorOfCorruption) == false) {
			...
			if ((player.horns.type == Horns.BICORN || player.horns.type == Horns.UNICORN) && player.cor < 11 && player.hasPerk(PerkLib.AvatorOfPurity) == false) {
			...
			if (player.hasPerk(PerkLib.Androgyny) == false) { //Fix femininity ratings if out of whack!
			...
			if (player.flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00285] >= 50 && player.hasPerk(PerkLib.LuststickAdapted) == false) { //Luststick resistance unlock
			...
			if (player.hasPerk(PerkLib.Oviposition) || player.hasPerk(PerkLib.BunnyEggs)) { //Oviposition perk for lizard and bunny folks
				if ((player.nagaScore() + player.lizardScore() < 3) && player.hasPerk(PerkLib.Oviposition) && player.hasPerk(PerkLib.BasiliskWomb) == false) { //--Lose Oviposition perk if lizard score gets below 3.
