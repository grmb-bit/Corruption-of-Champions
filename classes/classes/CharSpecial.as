...
/**
	...
	//		if(player.hasPerk(PerkLib.WizardsEndurance) == false) player.createPerk(PerkLib.WizardsEndurance,30,0,0,0);
			player.setWeapon(weapons.W_STAFF);
		...
		private function customEtis():void {
			...
			if (player.hasPerk(PerkLib.StrongBack) == false) { player.createPerk(PerkLib.StrongBack, 0, 0, 0, 0); player.itemSlot6.unlocked = true; }
			if (player.hasPerk(PerkLib.StrongBack2) == false) { player.createPerk(PerkLib.StrongBack2, 0, 0, 0, 0); player.itemSlot7.unlocked = true; }
			if (player.hasPerk(PerkLib.StrongBack3) == false) { player.createPerk(PerkLib.StrongBack3, 0, 0, 0, 0); player.itemSlot8.unlocked = true; }
			var i:int = 0;			
			...
			if (!player.hasPerk(PerkLib.PastLifeReligious) && player.hasPerk(PerkLib.HistoryReligious) == false) player.createPerk(PerkLib.HistoryReligious, 0, 0, 0, 0); // abandoned religion after obtaining nine tails and being disappointed in achieved enlightment
			if (!player.hasPerk(PerkLib.PastLifeFighter) && player.hasPerk(PerkLib.HistoryAlchemist) == false) player.createPerk(PerkLib.HistoryAlchemist, 0, 0, 0, 0); // and resorted to your hobby - alchemy
			if (player.hasPerk(PerkLib.TransformationResistance) == false) player.createPerk(PerkLib.TransformationResistance, 0, 0, 0, 0);  // tf resistance and alchemist are actually mutually nullifying each other - this is flavor mostly
			if (!player.hasPerk(PerkLib.PastLifeHealer) && player.hasPerk(PerkLib.HistoryHealer) == false) player.createPerk(PerkLib.HistoryHealer, 0, 0, 0, 0);  // with religious and alchemical knowlege you are skilled healer
			if (player.hasPerk(PerkLib.Medicine) == false) player.createPerk(PerkLib.Medicine, 0, 0, 0, 0); // able to treat wounds and poisoning alike
			if (player.hasPerk(PerkLib.AscensionMoralShifter) == false) player.createPerk(PerkLib.AscensionMoralShifter, 5, 0, 0, 0); // your kitsune's trickster nature makes you susceptable to corruption
			if (player.hasPerk(PerkLib.AscensionTolerance) == false) player.createPerk(PerkLib.AscensionTolerance, 10, 0, 0, 0); // but in the same time your enlightment keeps you from really turning to demon, so corruption level does not really affect you much
			if (player.hasPerk(PerkLib.Fast) == false) player.createPerk(PerkLib.Fast, 1, 0, 0, 0); // gaining speed is pain in ass... this one is not for history flavor
			if (player.hasPerk(PerkLib.Smart) == false) player.createPerk(PerkLib.Smart, 1, 0, 0, 0); // int is easy to get, just for history flavor
			if (player.hasPerk(PerkLib.Lusty) == false) player.createPerk(PerkLib.Lusty, 1, 0, 0, 0); // have a lust problem
			if (player.hasPerk(PerkLib.HotBlooded) == false) player.createPerk(PerkLib.HotBlooded, 20, 0, 0, 0); // even with your willpower and religious training you sometimes struggling to restrain your impulse
			if (player.hasPerk(PerkLib.Pervert) == false) player.createPerk(PerkLib.Pervert, 0.25, 0, 0, 0); // you always ready for something VERY lewd
			if (player.hasPerk(PerkLib.Masochist) == false) player.createPerk(PerkLib.Masochist, 0, 0, 0, 0); // with your knowlege of healing and innatural body it is easy for you to enjoy things which would be really painful for others
			if (player.hasPerk(PerkLib.Sadist) == false) player.createPerk(PerkLib.Sadist, 0, 0, 0, 0); // and you are always ready to return favor
			if (player.hasPerk(PerkLib.SensualLover) == false) player.createPerk(PerkLib.SensualLover, 0, 0, 0, 0); // still you tend to care about mutual enjoyment - there are difference between extreme entertainment and torture, and you are mischievous, not evil
			if (player.hasPerk(PerkLib.SpellcastingAffinity) == false) player.createPerk(PerkLib.SpellcastingAffinity, 80, 0, 0, 0); // very strong magic affinity, can even be effective as mage without robe
			if (player.hasPerk(PerkLib.Channeling) == false) player.createPerk(PerkLib.Channeling, 0, 0, 0, 0); // despite strong magic affinity wasn't actually much interested in combat magic before, so only can use fox fire as offensive spell
			if (player.hasPerk(PerkLib.Spellpower) == false) player.createPerk(PerkLib.Spellpower, 0, 0, 0, 0);
			// have some racial traits
			if (player.hasPerk(PerkLib.EnlightenedKitsune) == false) player.createPerk(PerkLib.EnlightenedKitsune, 0, 0, 0, 0);
			if (player.hasPerk(PerkLib.EnlightenedNinetails) == false) player.createPerk(PerkLib.EnlightenedNinetails, 0, 0, 0, 0);			
			if (player.hasPerk(PerkLib.DragonFireBreath) == false) player.createPerk(PerkLib.DragonFireBreath, 0, 0, 0, 0);				
			if (player.hasPerk(PerkLib.DragonIceBreath) == false) player.createPerk(PerkLib.DragonIceBreath, 0, 0, 0, 0);				
			if (player.hasPerk(PerkLib.DragonLightningBreath) == false) player.createPerk(PerkLib.DragonLightningBreath, 0, 0, 0, 0);				
			if (player.hasPerk(PerkLib.DragonDarknessBreath) == false) player.createPerk(PerkLib.DragonDarknessBreath, 0, 0, 0, 0);				
			// some experiments with your body gave unusual results
			if (player.hasPerk(PerkLib.LustyRegeneration) == false) player.createPerk(PerkLib.LustyRegeneration, 0, 0, 0, 0); // some of your experiments had nice returns
			if (player.hasPerk(PerkLib.Incorporeality) == false) player.createPerk(PerkLib.Incorporeality, 0, 0, 0, 0);	// some was... strange
			if (player.hasPerk(PerkLib.ChiReflowMagic) == false) player.createPerk(PerkLib.ChiReflowMagic, 0, 0, 0, 0);	// and some brought much more problems than benefits, but you wasn't able to reverse all of them. well, this is occupational hazard for alchemist
			// you have no idea how you got this... heritage, maybe?
			if (debug && player.hasPerk(PerkLib.FerasBoonAlpha) == false) player.createPerk(PerkLib.FerasBoonAlpha, 0, 0, 0, 0);			
			if (debug && player.hasPerk(PerkLib.FerasBoonBreedingBitch) == false) player.createPerk(PerkLib.FerasBoonBreedingBitch, 0, 0, 0, 0);		
