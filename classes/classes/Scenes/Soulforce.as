...
use namespace CoC;
	...
	public class Soulforce extends BaseContent
	{
		...
		*/	outputText("<b>Uses of soulforce per day (for 4 first option beside cultivate):</b> " + flags[kFLAGS.DAILY_SOULFORCE_USE_LIMIT] + " / " + dailySoulforceUsesLimit + "\n");
			...
			if (flags[kFLAGS.DAILY_SOULFORCE_USE_LIMIT] < dailySoulforceUsesLimit && player.hasPerk(PerkLib.SoulApprentice)) addButton(3, "Mana", ManaAndSoulforce).hint("Convert some soulforce into mana or vice versa."); //używanie soulforce do zamiany na mane w stosunku 1:1 a many do soulforce 1:2, używalne nawet w walce też ale z wiekszym kosztem przeliczania czyli 1:2 i 1:4
		...
		public function PerkGalore1():void {
			...
			if (player.hasPerk(PerkLib.Weap0n) && player.hasPerk(PerkLib.Arm0r) == false) {
				player.createPerk(PerkLib.Arm0r, 0, 0, 0, 0);
				outputText("\n\n<b>(Gained Perk: Armor!)</b>");
			}
			if (player.hasPerk(PerkLib.SexChampion) && player.hasPerk(PerkLib.SexDeity) == false) {
		...
		public function PerksGemsEXPLvL():void {
			...
			if (player.hasPerk(PerkLib.HclassHeavenTribulationSurvivor) == false) addButton(10, "Trib Perks", TribulationPerks).hint("Add 4 Tribulation perks.");
			if (player.hasPerk(PerkLib.SoulAncestor) == false) addButton(11, "10-12 St.", Stage10to12SoulPerks).hint("Add all soul cultivator related perks for stages 10-12 of cultivation.");
		...
		public function NonEquipmentMenu(page:int = 1):void {
			...
			if (page == 2) {
				...
				if (player.hasPerk(PerkLib.ElementalConjurerMindAndBodySacrifice) == false) addButton(12, "E.Pearls", AddThePearls).hint("Add all three Elemental Pearls.");
