...
private function yesPutDatSockOnMe(target:int):void {
	...
	if(flags[kFLAGS.SOCK_HOLDING] == "cobalt") {
		player.gems -= 250;
		// if(player.hasPerk(PerkLib.PhallicRestraint) == false) player.createPerk(PerkLib.PhallicRestraint,0,0,0,0);
	}

	if(flags[kFLAGS.SOCK_HOLDING] == "scarlet") {
		//if(player.hasPerk(PerkLib.PhallicPotential) == false) player.createPerk(PerkLib.PhallicPotential,0,0,0,0);
		player.gems -= 250;
	}

	if(flags[kFLAGS.SOCK_HOLDING] == "viridian") {
		if(player.hasPerk(PerkLib.LustyRegeneration) == false) {
	...
	if(flags[kFLAGS.SOCK_HOLDING] == "cockring") {
		player.gems -= 100;
		if(player.hasPerk(PerkLib.PentUp) == false) player.createPerk(PerkLib.PentUp,10,0,0,0);
