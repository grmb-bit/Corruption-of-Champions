...
public function benoitIntro():void {
	...
	//Basilisk Womb
	if(flags[kFLAGS.BENOIT_WOMB_TALK_UNLOCKED] == 1 && player.hasPerk(PerkLib.BasiliskWomb) == false && flags[kFLAGS.BENOIT_TESTED_BASILISK_WOMB] == 0 && flags[kFLAGS.BENOIT_STATUS] == 0) addButton(4, "Basil. Womb", tryToConvertToBassyWomb);
	//Suggest & sex
...
private function tryToConvertToBassyWomb():void {
	...
	//Ingredients in inventory: 
		...
		//[No oviposition:
		if(player.hasPerk(PerkLib.Oviposition) == false) outputText("  You feel slightly bloated, but otherwise fine; you sense that you can lay eggs of your own volition now.");
...
private function suggestSexAfterBasiWombed(later:Boolean = true):void {
	...
	if(player.hasPerk(PerkLib.Oviposition) == false) {
