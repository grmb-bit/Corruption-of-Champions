...
//[Beach]
		...
		{
			if (rand(4) == 0 && player.hasStatusEffect(StatusEffects.SlimeCraving) && player.hasPerk(PerkLib.SlimeCore) == false && player.hasPerk(PerkLib.DarkSlimeCore) == false && player.isGoo() && player.gooScore() >= 4) {
...
//Defeat – Female
		private function femaleLoseToGooGal():void
		{
			...
			//[If the player isn't pregnant]
				...
				if (player.pregnancyIncubation >= 1) outputText("  Everything she pumped into you runs back out after.  For some reason or another, it didn't take.");
				else {
