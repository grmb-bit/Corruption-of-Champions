...
//Damage victory
		public function defeatTheKitsunes(display:Boolean = true):void
		{
			...
			if (player.pcCanUseUniqueSexScene() && flags[kFLAGS.SFW_MODE] <= 0) addButton(13, "U. Sex Scenes", uniquuuesexscene.pcUniqueSexScenesChoiceMenu).hint("Other non typical sex scenes.");
...
//[Read Books]
		...
		public function meditateAtKitsuneShrine():int {
			...
			if (player.hasItem(consumables.FOXJEWL) && player.ears.type == Ears.FOX && player.tailType == Tail.FOX) {
				var notANineTail:Boolean = player.hasPerk(PerkLib.CorruptedNinetails) == false
										   || player.perkv4(PerkLib.CorruptedNinetails) > 0;
				if (flags[kFLAGS.KITSUNE_SHRINE_UNLOCKED] > 0 && player.tailCount >= 2 && player.hasPerk(PerkLib.StarSphereMastery) == false) {
					...
					player.tailCount = 9;
					player.consumeItem(consumables.FOXJEWL);
				} else if (player.tailCount == 7 && player.level >= 36 && player.inte >= 105 && player.wis >= 105 && notANineTail && (player.hasPerk(PerkLib.CorruptedKitsune) == false || player.perkv4(PerkLib.CorruptedKitsune) > 0)) {
					outputText("Nearing the end of your meditation, you are inexplicably compelled to reach into your bag and pull out the small teardrop-shaped jewel you were carrying.  As you stare past the translucent surface of the bead and into the dancing fire within, the jewel begins to dissolve in your hand, the pale flames within spilling out and spreading over your body.\n\n");
					...
					player.tailCount++;
					doNext(camp.returnToCampUseOneHour);
				} else if (player.tailCount == 6 && player.level >= 30 && player.inte >= 90 &&  player.inte >= 90 && notANineTail && (player.hasPerk(PerkLib.CorruptedKitsune) == false || player.perkv4(PerkLib.CorruptedKitsune) > 0)) {
					outputText("Nearing the end of your meditation, you are inexplicably compelled to reach into your bag and pull out the small teardrop-shaped jewel you were carrying.  As you stare past the translucent surface of the bead and into the dancing fire within, the jewel begins to dissolve in your hand, the pale flames within spilling out and spreading over your body.\n\n");
