		...
		public function giantGrabFail(struggle:Boolean = true):void {
			...
			if (struggle) {
				...
				if (player.str >= 200) {
					...
					if (player.hasPerk(PerkLib.Juggernaut) == false && armorPerk != "Heavy") {damage = 30 + rand((str + weaponAttack) * 1.5);
					player.takePhysDamage(damage, true);
					}
				}
					...
					if (player.hasPerk(PerkLib.Juggernaut) == false && armorPerk != "Heavy") {damage = 60 + rand((str + weaponAttack) * 2.25);
					player.takePhysDamage(damage, true);
					}
				}
