...
public function minoCumUpdate():Boolean {
	...
	if(flags[kFLAGS.MINOTAUR_CUM_ADDICTION_TRACKER] < 50) {
		...
		if (flags[kFLAGS.MINOTAUR_CUM_RESISTANCE_TRACKER] >= 150 && player.hasPerk(PerkLib.MinotaurCumAddict) == false && player.hasPerk(PerkLib.MinotaurCumResistance) == false) {
	...
	//If over 50, addicted, become addicted, withdrawl, etc
		...
		if(flags[kFLAGS.MINOTAUR_CUM_ADDICTION_TRACKER] >= 100 && player.hasPerk(PerkLib.MinotaurCumResistance) == false && player.hasPerk(PerkLib.ManticoreCumAddict) == false) {
	...
	if(flags[kFLAGS.TIME_SINCE_LAST_CONSUMED_MINOTAUR_CUM] >= 48 && player.hasPerk(PerkLib.MinotaurCumAddict) == false) {
