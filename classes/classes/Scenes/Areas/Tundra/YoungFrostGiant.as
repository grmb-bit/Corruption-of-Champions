		...
		public function youngGiantGrabFail(struggle:Boolean = true):void {
			...
			if (struggle) {
				...
				if (player.str >= 80) {
					...
					if (player.hasPerk(PerkLib.Juggernaut) == false && armorPerk != "Heavy") {damage = 10 + rand((str + weaponAttack) * 0.5);
					player.takePhysDamage(damage, true);
					}
				}
					...
					if (player.hasPerk(PerkLib.Juggernaut) == false && armorPerk != "Heavy") {damage = 20 + rand((str + weaponAttack) * 0.75);
					player.takePhysDamage(damage, true);
					}
				}
