...
// TIMES_EMBER_LUSTY_FUCKED:int = 824;
		...
		public function timeChange():Boolean
		{
			...
			if (followerEmber() && !player.hasStatusEffect(StatusEffects.EmberNapping)) {
				...
				if (flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] == 1 && player.hasPerk(PerkLib.MinotaurCumAddict) == false && flags[kFLAGS.EMBER_CURRENTLY_FREAKING_ABOUT_MINOCUM] == 0) {
...
//Get Milk
		private function getMilkFromEmber():void
		{
			...
			if (emberAffection() <= 25) {
				...
				if ((player.hasStatusEffect(StatusEffects.DragonBreathCooldown) && player.hasPerk(PerkLib.DraconicLungsEvolved) == false) || ((player.hasStatusEffect(StatusEffects.DragonDarknessBreathCooldown) || player.hasStatusEffect(StatusEffects.DragonFireBreathCooldown) || player.hasStatusEffect(StatusEffects.DragonIceBreathCooldown) || player.hasStatusEffect(StatusEffects.DragonLightningBreathCooldown)) && player.hasPerk(PerkLib.DraconicLungs) == false)) {
