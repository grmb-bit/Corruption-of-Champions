...
import classes.internals.*;
	...
	public class Dinah extends Monster
	{
		...
		public function SoulskillMod():Number {
			var mod1:Number = 1;
			if (hasPerk(PerkLib.DaoistCultivator)) mod1 += .1;
			if (hasPerk(PerkLib.DaoistApprenticeStage)) {
				if (hasPerk(PerkLib.SoulApprentice)) mod1 += .2;
				if (hasPerk(PerkLib.SoulPersonage)) mod1 += .2;
				if (hasPerk(PerkLib.SoulWarrior)) mod1 += .2;
			}
			if (hasPerk(PerkLib.DaoistWarriorStage)) {
				if (hasPerk(PerkLib.SoulSprite)) mod1 += .3;
				if (hasPerk(PerkLib.SoulScholar)) mod1 += .3;
				if (hasPerk(PerkLib.SoulElder)) mod1 += .3;
			}
			return mod1;
		}
