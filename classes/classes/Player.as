...
use namespace CoC;
	...
	public class Player extends Character {
		...
		{
			return (((isTaur() || isDrider() || canFly()) && spe >= 60) && hasPerk(PerkLib.Naturaljouster) && (hasPerk(PerkLib.DoubleAttack) == false || (hasPerk(PerkLib.DoubleAttack) && flags[kFLAGS.DOUBLE_ATTACK_STYLE] == 0)))
             || (spe >= 150 && hasPerk(PerkLib.Naturaljouster) && hasPerk(PerkLib.DoubleAttack) && (hasPerk(PerkLib.DoubleAttack) == false || (hasPerk(PerkLib.DoubleAttack) && flags[kFLAGS.DOUBLE_ATTACK_STYLE] == 0)));
		}
		public function isMeetingNaturalJousterMasterGradeReq():Boolean
		{
			return (((isTaur() || isDrider() || canFly()) && spe >= 180) && hasPerk(PerkLib.NaturaljousterMastergrade) && (hasPerk(PerkLib.DoubleAttack) == false || (hasPerk(PerkLib.DoubleAttack) && flags[kFLAGS.DOUBLE_ATTACK_STYLE] == 0)))
             || (spe >= 450 && hasPerk(PerkLib.NaturaljousterMastergrade) && hasPerk(PerkLib.DoubleAttack) && (hasPerk(PerkLib.DoubleAttack) == false || (hasPerk(PerkLib.DoubleAttack) && flags[kFLAGS.DOUBLE_ATTACK_STYLE] == 0)));
		}
		...
		override public function get weaponAttack():Number {
			...
			if (hasStatusEffect(StatusEffects.ChargeWeapon)) {
				if (weaponName == "fists" && hasPerk(PerkLib.ImprovingNaturesBlueprintsNaturalWeapons) == false) attack += 0;
...
//				grandchimeraCounter++;
		...
		public function gorgonScore():Number {
			...
			if ((hasPerk(PerkLib.GorgonsEyesEvolved) || hasPerk(PerkLib.VenomGlandsEvolved)) && hasPerk(PerkLib.ChimericalBodySemiSuperiorStage))
				gorgonCounter++;
			//if ((hasPerk(PerkLib.) || hasPerk(PerkLib.VenomGlandsFinalForm)) && hasPerk(PerkLib.ChimericalBodySemiEpicStage))
			//	gorgonCounter++;
		...
		public function minoCumAddiction(raw:Number = 10):void {
			...
			if(raw > 50) raw = 50;
			if(raw < -50) raw = -50;
			if(hasPerk(PerkLib.ManticoreCumAddict) == false || hasPerk(PerkLib.LactaBovineImmunity) == false) flags[kFLAGS.MINOTAUR_CUM_ADDICTION_TRACKER] += raw;
		...
		public function shrinkTits(ignore_hyper_happy:Boolean=false):void
		{
			...
			if(breastRows.length == 1) {
				if(breastRows[0].breastRating > 0) {
					...
					if(breastRows[0].breastRating >= 1 && rand(2) == 0 && hasPerk(PerkLib.BigTits) == false) {
						temp++;
						breastRows[0].breastRating--;
					}
		...
		public function growTits(amount:Number, rowsGrown:Number, display:Boolean, growthType:Number):void
		{
			...
			if(growthType == 1) {
				...
				while(rowsGrown > 0) {
					...
					if (!flags[kFLAGS.HYPER_HAPPY])
					{
						...
						{
							if(hasPerk(PerkLib.BigTits) == false)
								temp /=1.5;
							else
								temp /=1.3;
						}
			...
			if (!flags[kFLAGS.HYPER_HAPPY])
			{
				...
				if(breastRows[0].breastRating > 3) {
					if(hasPerk(PerkLib.BigTits) == false) amount/=1.5;
					else amount/=1.3;
				}
				if(breastRows[0].breastRating > 7) {
					if(hasPerk(PerkLib.BigTits) == false) amount/=2;
					else amount /= 1.5;
				}
		...
		public override function minLust():Number
		{
			...
			if(hasPerk(PerkLib.HotBlooded)) min += getPerk(PerkLib.HotBlooded).value1;
			...
			//Werebeast
			if (hasPerk(PerkLib.Lycanthropy)) min += getPerk(PerkLib.Lycanthropy).value1;
			//Jewelry effects
		...
		*/	if (beeScore() >= 5) {
		...
		public function minotaurAddicted():Boolean {
			return hasPerk(PerkLib.MinotaurCumResistance) == false && hasPerk(PerkLib.ManticoreCumAddict) == false && (hasPerk(PerkLib.MinotaurCumAddict) || flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] >= 1);
		}
		public function minotaurNeed():Boolean {
			return hasPerk(PerkLib.MinotaurCumResistance) == false && hasPerk(PerkLib.ManticoreCumAddict) == false && flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] > 1;
		}
...
*/
		...
		override public function modStats(dstr:Number, dtou:Number, dspe:Number, dinte:Number, dwis:Number, dlib:Number, dsens:Number, dlust:Number, dcor:Number, scale:Boolean, max:Boolean):void {
			...
			if (scale) {
				...
				if (hasPerk(PerkLib.Strong)) dstr += dstr * getPerk(PerkLib.Strong).value1;
				if (hasPerk(PerkLib.Tough)) dtou += dtou * getPerk(PerkLib.Tough).value1;
				if (hasPerk(PerkLib.Fast)) dspe += dspe * getPerk(PerkLib.Fast).value1;
				if (hasPerk(PerkLib.Smart)) dinte += dinte * getPerk(PerkLib.Smart).value1;
				if (hasPerk(PerkLib.Wise)) dwis += dwis * getPerk(PerkLib.Wise).value1;
				if (hasPerk(PerkLib.Lusty)) dlib += dlib * getPerk(PerkLib.Lusty).value1;
				if (hasPerk(PerkLib.Sensitive)) dsens += dsens * getPerk(PerkLib.Sensitive).value1;
